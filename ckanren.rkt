...
(define (value?δ v)
  (and (integer? v) (<= 0 v)))
(define (memv?δ v δ)
  (and (value?δ v) (memv v δ)))
(define null?δ null?)
(define (singleton?δ δ)
  (null? (cdr δ)))
...
(define (disjoint?δ δ1 δ2)
  (cond
    ...
    [(= (car δ1) (car δ2)) #f]
    [(< (car δ1) (car δ2)) (disjoint?δ (cdr δ1) δ2)]
    [else (disjoint?δ δ1 (cdr δ2))]))
(define (diffδ δ1 δ2)
  (cond
    [(or (null? δ1) (null? δ2)) δ1]
    [(= (car δ1) (car δ2)) (diffδ (cdr δ1) (cdr δ2))]
    [(< (car δ1) (car δ2)) (cons (car δ1) (diffδ (cdr δ1) δ2))]
    [else (diffδ δ1 (cdr δ2))]))
(define (intersectionδ δ1 δ2)
  (cond
    [(or (null? δ1) (null? δ2)) '()]
    [(= (car δ1) (car δ2)) (cons (car δ1) (intersectionδ (cdr δ1) (cdr δ2)))]
...
(define (verify-all-bound c bound-x*)
  (unless (null? c)
    (cond
      [(find (λ (x) (not (memq x bound-x*))) (filter var? (oc->rands (car c))))
       =>
       (λ (x) (error 'verify-all-bound "Constrained variable ~s without domain" x))]
...
(define (run-constraints x* c)
  (cond
    ...
    [(any-relevant/var? (oc->rands (car c)) x*)
     (composeM (rem/run (car c)) (run-constraints x* (cdr c)))]
...
(define-syntax build-auxoc
  (syntax-rules ()
    [(_ op () (z ...) (arg ...)) (let ([z arg] ...) `(,(op z ...) . (op ,z ...)))]
    [(_ op (arg0 arg ...) (z ...) args) (build-auxoc op (arg ...) (z ... q) args)]))
(define-syntax buildoc
  (syntax-rules ()
    [(_ op arg ...) (build-auxoc op (arg ...) () (arg ...))]))
...
(define (domfdc x n*)
  (λM (a : s d c) ((processδ (walk x s) (makeδ n*)) a)))
...
(define (+fdc u v w)
  (c-op +fdc
        ...
        (let ([umin (minδ uδ)]
              ...
              [wmin (minδ wδ)]
              [wmax (maxδ wδ)])
          (composeM (processδ w (range (+ umin vmin) (+ umax vmax)))
                    (composeM (processδ u (range (- wmin vmax) (- wmax vmin)))
                              (processδ v (range (- wmin umax) (- wmax umin))))))))
...
(define (exclude-fromδ δ1 d x*)
  (let loop ([x* x*])
    (cond
      ...
      [(getδ (car x*) d)
       =>
       (λ (δ2) (composeM (processδ (car x*) (diffδ δ2 δ1)) (loop (cdr x*))))]
      [else (loop (cdr x*))])))
        ...
        (cond
          ...
          [else
           (let-values ([(x* n*) (partition var? v*)])
             (let ([n* (list-sort < n*)])
               (cond
                 [(list-sorted? < n*) ((all-diff/fdc x* n*) a)]
                 [else #f])))]))))
      ...
      (let ([x (walk x s)])
        ((cond
           [(and (var? x) (getδ x d))
            =>
            (map-sum (λ (v) (== x v)))]
           [(pair? x) (fresh () (force-ans (car x)) (force-ans (cdr x)))]
           [else succeed])
         a))))
...
(define-syntax condu
  (syntax-rules ()
    [(_ (g0 g ...) (g1 ĝ ...) ...) (λG (a) (inc (ifu ((g0 a) g ...) ((g1 a) ĝ ...) ...)))]))
(define (onceo g)
  (condu (g)))
